...
export async function checkOrganic({base,connection,evaluate,click,fill,text,waitFor,screenshot}) {
  ...
  await change('#sl-shelf-group','Acids');assert.match(await text('.sl-reagents'),/No matches/);await click('[data-lab="shelf-clear"]');assert.equal(await evaluate('document.querySelectorAll(".sl-reagent").length'),76);
