...
The shared chemical shelf contains **76 stocks and reference entries**, including **12 common dry materials**: sodium chloride, sodium bicarbonate, sucrose, citric acid, chalk, starch, silica sand, iron filings, magnesium ribbon, zinc granules, activated carbon and copper(II) sulfate pentahydrate. The **Electrochemistry** collection adds supporting salts, dilute sulfuric acid and 11 electrode surfaces. **Mineral analysis & standards** contains 13 entries: the unknown concentrate, analytical reagents, virtual treatment packs and Ag/Au/Pt/REE controls. Search by name/formula, filter by collection and category, or scroll the bounded list.
...
- On the **Vessel bench**, Guided mode shows instructions and optional discussion; Student removes those prompts; Assessment hides guidance, conceptual contents, equations, calculation feedback and answer tools. Professor can reveal the aqueous unknown and create/edit structured JSON study definitions using existing chemistry rules. Free exploration changes the objective to your own question and leaves equipment available. **Mineral analysis** shows optional step advice in Guided; Student, Free and Professor omit that advice while retaining eligible feedback. Assessment additionally hides equations, generated conclusions, new instrument submissions and simulator answers in views and learning reports. Recorded observations, earlier instrument readings, learner conclusions and safety controls remain available. **Open research exceptions:** organic tools use an open research view; electrochemistry shows preset guidance in Guided mode but retains analytical results in other modes. See [ADR-0004](docs/adr/0004-learning-mode-contract.md). These are local learning modes, not a secure exam or authenticated teacher system.
...
1. Homogenise the concentrate, tare an empty container, then **Weigh & transfer** each of the six labelled portions. Finish preparation with Original and E preserved. Optionally enable uneven Au/Pt particle distribution.
2. For each stage in A, B, C and D, choose the apparatus and labelled sample/fraction, then **Load the selected fraction**. Guided mode explains the setup. Choose the required reagent from the shared shelf and add a model charge; contained treatments require the shared virtual hood.
3. Operate the equipment: mix, seat a filter and label both receivers, prepare individual blank/reference/sample/spike vials, focus the microscope, measure radiation background, or zero the spectrophotometer with its reagent blank. Change the model charge, relative interval or test-portion size to explore their effects.
4. Press the apparatus's **Run**, **Filter**, **React**, **Capture** or **Count** action to produce evidence. Inspect the drawing and results, then open **Record these observations**, write your interpretation and save. Compare the controls before recording each route's confidence and reason.
...
The tools include an illustrated balance, labelled fractions, filtration, contained digestion modules, microscope, radiation screening, test vials, a spectrophotometer for uncalibrated Arsenazo III comparisons, XRF, ICP-OES, ICP-MS, fire assay with a finish, and SEM-EDS. Apparatus setup survives equipment changes and reloads. **Return sample & clear this setup** clears the current staging before treatment; Undo restores earlier operations. Performing the analysis advances the model clock once; drawings do not run a background reaction timer. The recorded procedure includes the apparatus actions. Observations and reports save to the shared notebook and export as Markdown/CSV. No colour is converted to a grade. Chemical hazards and numerical assumptions are explained in [MINERAL_MODEL.md](MINERAL_MODEL.md); model charge units and intervals are teaching controls, not real operating recipes. [ADR-0006](docs/adr/0006-hands-on-mineral-apparatus.md) records the interactive workflow.

**Conclusion drafts save per aliquot:** confidence, reasoning and the control-comparison checkbox survive route, report, equipment and notebook navigation, learning-mode changes and reloads. Choose a confidence explicitly and press **Record conclusion** when ready. Invalid submissions retain the draft; Undo restores the preceding operation's draft; a full restart clears working drafts. Markdown and notebook JSON exports label unfinished drafts separately, while snapshots capture completed evidence and conclusions. See [ADR-0005](docs/adr/0005-mineral-conclusion-drafts.md).
...
The [architecture decision records](docs/adr/README.md) explain the shared workbench, model boundaries, mineral evidence workflow and implemented review fixes for learning modes and conclusion drafts.
...
| `src/mineral-protocol.js`, `src/mineral-workbench.js` | Apparatus protocols, staged preparation, supported shelf reagents and original interactive bench drawings |
