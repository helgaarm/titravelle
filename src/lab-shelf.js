...
export function shelfView(items,{query='',group='all',scope='all',selected=''}={}) {
  ...
  return `<div class="sl-section-head"><h2>Chemical shelf</h2><span>${items.length} stocks & references</span></div>
    ...
    <label>Collection<select id="sl-shelf-scope">${[['all','All materials'],['aqueous','Aqueous stocks'],['organic','Organic materials & references'],['dry','Common dry materials'],['electro','Electrochemistry materials']].map(([id,label])=>`<option value="${id}" ${id===scope?'selected':''}>${label}</option>`).join('')}</select></label>
