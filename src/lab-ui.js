...
export function startLab() {
  ...
  function plot() {
    const trial = state.vessels[graphVessel].trial || 0;
  ...
  function equationResults(compact = false) {
    ...
    return `<div class="sl-equation-report ${compact ? 'sl-equation-compact' : ''}"><p class="sl-equation-summary">${esc(report.summary)}</p>
      ...
      <div class="sl-reaction-cards">${report.reactions.map(r=>`<article class="sl-reaction-card" data-reaction="${r.id}"><div class="sl-reaction-title"><h3>${esc(r.title)}</h3><span class="sl-balanced">${r.balanced ? 'Atoms & charge balanced' : 'Balance unavailable'}</span></div><span class="sl-small">Net ionic equation</span><p class="sl-generated-equation">${esc(r.equation)}</p>
        ...
        <p class="sl-reaction-amount"><strong>${amount(r.moles)} mmol</strong> · ${esc(r.quantity)} · ${r.steps} ${r.steps===1?'step':'steps'}</p>${compact ? '' : `<p class="sl-small">${esc(r.detail)}</p>`}</article>`).join('')}</div>
